...
const searchSysUserFields = ['first_name', 'last_name', 'email'];

const getSearchFieldsObj = searchSysUserFields => {
  let searchFieldsObj = {};
  for (let field of searchSysUserFields) {
...
module.exports.up = async function() {
  ...
  const searchSysUserFieldsObj = getSearchFieldsObj(searchSysUserFields);
  await SystemUsers.collection.createIndex(searchSysUserFieldsObj);
  await db.disconnect();
};

module.exports.down = async function() {
  const db = await connectDb();
  await SystemUsers.collection.dropIndexes();
  await db.disconnect();
};
